...
/**
 * An http {@link com.google.android.exoplayer.upstream.DataSource}.
 */
public class RawHttpDataSource implements DataSource {

  /**
   * A {@link com.google.android.exoplayer.util.Predicate} that rejects content types often used for pay-walls.
   */
  ...
  private static final String TAG = "RawHttpDataSource";
  ...
  public void close() throws HttpDataSourceException {
    ...
    try {
      if (inputStream != null) {
        try {
            Log.d(TAG, "close(): inputStream.close()..., avail="+inputStream.available());
          inputStream.close();
            Log.d(TAG, "close(): inputStream.close()ok");
        } catch (IOException e) {
          throw new HttpDataSourceException(e, dataSpec);
        }
        inputStream = null;
      ...
      if (opened) {
        ...
        if (listener != null) {
          ...
          Log.d(TAG, "close(): closeConnection");
        closeConnection();
      }
    }
    Log.d(TAG, "close(): <--");
  ...
  /**
   * Returns the number of bytes that are still to be read for the current {@link com.google.android.exoplayer.upstream.DataSpec}.
   * <p>
   * If the total length of the data being read is known, then this length minus {@code bytesRead()}
   * is returned. If the total length is unknown, {@link com.google.android.exoplayer.C#LENGTH_UNBOUNDED} is returned.
   *
   * @return The remaining length, or {@link com.google.android.exoplayer.C#LENGTH_UNBOUNDED}.
   */
