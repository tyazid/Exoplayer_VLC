...
/* package */ class H264Reader extends ElementaryStreamReader {
            ...
            while (offset < limit) {
                ...
                if (nextNalUnitOffset < limit) {
                    ...
                    if (nalUnitType == NAL_UNIT_TYPE_AUD) {
                        if( log_enabled ) Log.d("H264Reader", "consume(): ...nalUnitType == NAL_UNIT_TYPE_AUD, "+getNalType(nalUnitType));
                        if (writingSample()) {
                            if( log_enabled ) Log.d("H264Reader", "consume(): ...isKeyframe="+isKeyframe);
                            if( log_enabled ) Log.d("H264Reader", "consume(): ...hasMediaFormat="+hasMediaFormat());
                            if( log_enabled ) Log.d("H264Reader", "consume(): ...sps.isCompleted="+sps.isCompleted());
                            if( log_enabled ) Log.d("H264Reader", "consume(): ...pps.isCompleted="+pps.isCompleted());
                        ...
                        isKeyframe = false;
                    } else if (nalUnitType == NAL_UNIT_TYPE_IDR ||nalUnitType==1) {
                        if( log_enabled ) Log.d("H264Reader", "consume(): ...nalUnitType == NAL_UNIT_TYPE_IDR, "+getNalType(nalUnitType));
                        isKeyframe = true;
                    }
                    else {
                        if( log_enabled ) Log.d("H264Reader", "consume(): ...nalUnitType == " + nalUnitType + ", "+getNalType(nalUnitType));
                    }
    ...
    public String getNalType(int naltype)
    {
        switch (naltype) {
            case 1:
                return "Slice layer without partitioning non IDR";
            case 2:
                return "Slice data partition A layer";
            case 3:
                return "Slice data partition B layer";
            case 4:
                return "Slice data partition C layer";
            case 5:
                return "Slice layer without partitioning IDR";
            case 6:
                return "Additional information (SEI)";
            case 7:
