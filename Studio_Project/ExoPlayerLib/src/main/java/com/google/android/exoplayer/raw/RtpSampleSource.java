 ...
 */
public class RtpSampleSource implements DataSource {

    private final String TAG = "RtpSampleSource";
    private final int BUFFER_LENGTH = 8*1024*1024;
    private final int SAMPLE_LENGTH = 4*1024;
    private final int RTP_HEADER_LEN = 12;          // RTP header packet length
    ...
    public RtpSampleSource(DataSource dataSource) { this.dataSource = dataSource; }
    ...
    */
    ...
    public long open(DataSpec dataSpec) throws IOException {
        ...
        Log.d(TAG, "open(): --> <--"+ Arrays.toString(Thread.currentThread().getStackTrace()));
    ...
    public int read(byte[] buffer, int offset, int readLength) throws IOException {
        ...
        synchronized(LOCK) {
            if( write_pos == read_pos ) {
                //Log.d(TAG, "Not enough data to read from, readLength="+readLength+", write_pos-read_pos="+(write_pos-read_pos)+", available="+content_length);
            }
    ...
    */
    ...
    private int internalReadFromSource() throws IOException {
        ...
        // Get sequence number of the RTP packet
        int seq_num = (tmp[4] & 0xFF)*256 + (tmp[3] & 0xFF);
        // Remove rtp headers
        read -= RTP_HEADER_LEN;

        this.packet_length = read;

        //Log.d("RtpSampleSource", "internalReadFromSource(): read "+(read/188)+" TS packet, seq="+seq_num+", used="+getContentLength()+" ("+(tmp[3]& 0xFF)+"."+(tmp[4]& 0xFF)+")");

        // lost packet detection based on the sequence number.
        // TODO: in this case we would like to return a discontinuity to the caller
        if( seq_num > last_packet_nb+1  )
            Log.e(TAG, "internalReadFromSource(): lost "+(seq_num-(last_packet_nb+1))+" packets");
        last_packet_nb = seq_num;

        synchronized(LOCK) {
            if( write_pos >= read_pos ) {
                int len = Math.min(BUFFER_LENGTH-write_pos, read);
                System.arraycopy(tmp, RTP_HEADER_LEN, this.buffer, write_pos, len);
                write_pos += len;
                ...
                if( len < read ) {
                    ...
                    if( rest < read-len ) // TODO: in this case we would like to return a discontinuity to the caller
                        Log.e(TAG, "internalReadFromSource(): can't write all data... Buffer full? (1)");
                    System.arraycopy(tmp, len+RTP_HEADER_LEN, this.buffer, write_pos, rest);
                    write_pos += rest;
                }
            }
                ...
                if( len < read ) // TODO: in this case we would like to return a discontinuity to the caller
                    Log.e(TAG, "internalReadFromSource(): can't write all data... Buffer full? (2)");
                System.arraycopy(tmp, RTP_HEADER_LEN, this.buffer, write_pos, len);
                write_pos += len;
            }
        }
    ...
    void skipOlderPacket()
    {
        ...
        Log.d(TAG, "skipOlderPacket(): free "+this.packet_length+" bytes");
        int to_read;
        if( write_pos < read_pos ) {
